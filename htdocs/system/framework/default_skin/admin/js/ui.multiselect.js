   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	...
	_applyItemState: function(item, selected) {
		if (selected) {
			...
			this._registerRemoveEvents(item, item.find('a.action'));
			
		} else {
			item.children('span').addClass('ui-helper-hidden').removeClass('ui-icon');
			item.find('a.action span').addClass('ui-icon-plus').removeClass('ui-icon-minus');
			this._registerAddEvents(item, item.find('a.action'));
		}
	...
	_registerAddEvents: function(liElem, elements) {
		var that = this;
		...
		liElem.unbind('dblclick.multiselect').bind('dblclick.multiselect', function() {
			// ignore if busy...
			if (!this.busy) {
	...
	_registerRemoveEvents: function(liElem, elements) {
		var that = this;
		elements.unbind('click.multiselect').bind('click.multiselect', function() {
			// ignore if busy...
			if (!that.busy) {

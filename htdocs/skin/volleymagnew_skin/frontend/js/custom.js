...
(function () {
    function AddListeners(openBtn, closeBtn, popupElement) {
        ...
        document.querySelector(".popup_overlay").addEventListener('click', toggler);

        function toggler(e) {
            // popupElementRef.classList.toggle('hide');

            if(!e.target.closest("." + popupElement) || e.target.closest("." + openBtn) || e.target.classList.contains(closeBtn) || e.target.classList.contains('popup_overlay')) {
                popupElementRef.classList.toggle('hide');
            }
        }
    }
    ...
    //header sticky
    window.addEventListener('scroll', initHeaderSticky);
    function initHeaderSticky() {

        var doc = document.documentElement,
            header = doc.querySelector('.main-header');
        ...
        if(top > headerH) {
            ...
            header.style.paddingTop = headerH + 'px';
        } else {
    ...
    //mobile nav toggle
    headerMenuToggle('.header_nav-btn', '.header_menu');
    headerMenuToggle('.categories-btn', '.header_categories_menu');

    function headerMenuToggle(btnToggle, container) {
        document.querySelector(btnToggle).addEventListener('click', function (e) {

            if(document.querySelector(container).classList.contains('is-active')) {
                $(container).slideUp();
        ...
        document.addEventListener('click', function (e) {
            ...
            if(!e.target.closest(container) && !e.target.closest(btnToggle)) {
                $(container).slideUp();
                document.querySelector(container).classList.remove('is-active');
            }
        });
    ...
    //make active nav menu category item
    setActiveNavCategory();
    function setActiveNavCategory() {

        var path = window.location.pathname;
        var el = $('a[href="' + path + '"]');
        el.closest('li').addClass('active').closest('.header_category-item').addClass('active').find('.header_subcategory').css({'display': 'block'});

        $(document).on('click', '.header_collapse-anchor', function (e) {
            var parent = $(this).closest('.has-submenu');

            if(parent.length) {
                e.preventDefault();

                parent.siblings().removeClass('active').find('.header_subcategory').slideUp();

                parent.toggleClass('active').find('.header_subcategory').slideToggle();
            }

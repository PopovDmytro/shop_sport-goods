...
class dBug{
	...
	function getVariableName() {
		...
		for($i=sizeof($arrBacktrace)-1; $i>=0; $i--) {
			$arrCurrent = $arrBacktrace[$i];
			if(array_key_exists("function", $arrCurrent) && 
				(in_array($arrCurrent["function"], $arrInclude) || strtolower($arrCurrent["function"]) != "dbug")) {
				continue;
			}
			$arrFile = $arrCurrent;
			break;
    ...
    {
        $this->makeTableHeader("array", "undefined");
	...
	function initJSandCSS() {
		echo <<<SCRIPTS
			...
			function show_item(evt) {
		        var obj=document.getElementById(evt);
		        if (obj.className == "") {
		            obj.className += " over"; 
		        } else {
		            obj.className = "";
			...
			<style type="text/css">
				...
				.debug {background-color:#ffffff; margin: 0;}

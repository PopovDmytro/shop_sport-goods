...
*
*/
class RexIni extends RexObject
{
    ...
    */
    ...
    protected static function getIniString(&$ini_array, $prefix = '')
    {
        $result = '';
        foreach ($ini_array as $key => &$value) {
            if (is_array($value)) {
                $result .= static::getIniString($value, $prefix.$key.'.')."\n";
            } else {
                $result .= $prefix.$key." = '".addslashes($value)."'\n";
            }
        }
        return $result;
    }

    public static function write($filename, $ini_array, $has_sections = true)
    { 
        $content = "";
        if ($has_sections) {
            foreach ($ini_array as $section => $section_array) { 
                $content .= "[".$section."]\n";
                $content .= static::getIniString($section_array);
            }
        } else { 
            $content .= static::getIniString($ini_array);
        }

        //var_dump($content);
        //exit;
        if (!file_put_contents($filename, $content)) {
            throw new Exception('Write error to file "'.$filename.'"');
        }
    }
}
...
{/if}
  ...
  <table width="100%" cellpadding="2" cellspacing="1" class="data_column" border="0">
    <tr class="{if $flagLinesColored}Odd{/if}">
    {assign var  = columnNum value=1}
        <!-- Build body -->
        {section name=row loop=$recordSet}
          {math assign = count equation="y % x" x=$columns.count y=$smarty.section.row.iteration}

              <td {$columns.attributes}>
            ...
            {if $count == 0}
                {assign var = columnNum value=$columnNum+1}
                  </tr><tr class="{if $flagLinesColored}{if $columnNum is even}Even{else}Odd{/if}{else}Odd{/if}">
            {/if}

...
require_once('../../components/XPear/pear/PEAR.php/system/components../../components/XPear/pear/Config/Container.phponents/Container.php_once('Config/Container.php');
    ...
    function registerConfigType($configType, $configInfo = false)
    {
        if (Config::isConfigTypeRegistered($configType)) {
            ...
            if ($info[0] == $configInfo[0] &&
                ...
                return PEAR::staticRaiseError("Config::registerConfigType registration of existing $configType failed.", null, PEAR_ERROR_RETURN);
            }
        }
        ...
        if ($file_exists) {
            if (!class_exists($configInfo[1])) {
                return PEAR::staticRaiseError("Config::registerConfigType class '$configInfo[1]' not found in $configInfo[0]", null, PEAR_ERROR_RETURN);
            }
        } else {
            return PEAR::staticRaiseError("Config::registerConfigType file $configInfo[0] not found", null, PEAR_ERROR_RETURN);
        }
    ...
    */
    function setRoot(&$rootContainer)
    {
        if (is_object($rootContainer) && strtolower(get_class($rootContainer)) === 'config_container') {
            ...
            return PEAR::staticRaiseError("Config::setRoot only accepts object of Config_Container type.", null, PEAR_ERROR_RETURN);
        }
    } // end func setRoot
    ...
    */
    function &parseConfig($datasrc, $configType, $options = array())
    {
        ...
        if (!$this->isConfigTypeRegistered($configType)) {
            return PEAR::staticRaiseError("Configuration type '$configType' is not registered in Config::parseConfig.", null, PEAR_ERROR_RETURN);
        }

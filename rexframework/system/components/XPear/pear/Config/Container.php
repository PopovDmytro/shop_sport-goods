...
class Config_Container {
    ...
    */
    function &addItem(&$item, $where = 'bottom', $target = null)
    {
        if ($this->type != 'section') {
            return PEAR::staticRaiseError('Config_Container::addItem must be called on a section type object.', null, PEAR_ERROR_RETURN);
        }
        if (is_null($target)) {
            $target = $this;
        }
        if (strtolower(get_class($target)) != 'config_container') {
            return PEAR::staticRaiseError('Target must be a Config_Container object in Config_Container::addItem.', null, PEAR_ERROR_RETURN);
        }
        ...
        switch ($where) {
            ...
            case 'bottom':
                ...
                break;
            default:
                return PEAR::staticRaiseError('Use only top, bottom, before or after in Config_Container::addItem.', null, PEAR_ERROR_RETURN);
        }
    ...
    */
    ...
    {
        if ($this->type != 'section') {
            return PEAR::staticRaiseError('Config_Container::getItem must be called on a section type object.', null, PEAR_ERROR_RETURN);
        }
        if (!is_null($type)) {
            $testFields[] = 'type';
        }
    ...
    */
    ...
    {
        if ($this->type != 'section') {
            return PEAR::staticRaiseError('Config_Container::searchPath must be called on a section type object.', null, PEAR_ERROR_RETURN);
        }
    ...
    */
    ...
    {
        if ($this->isRoot()) {
            return PEAR::staticRaiseError('Cannot remove root item in Config_Container::removeItem.', null, PEAR_ERROR_RETURN);
        }
    ...
    */
    function toString($configType, $options = array())
    {
        ...
        if (!isset($GLOBALS['CONFIG_TYPES'][$configType])) {
            return PEAR::staticRaiseError("Configuration type '$configType' is not registered in Config_Container::toString.", null, PEAR_ERROR_RETURN);
        }
    ...
    */
    function writeDatasrc($datasrc, $configType, $options = array())
    {
        $configType = strtolower($configType);
        if (!isset($GLOBALS['CONFIG_TYPES'][$configType])) {
            return PEAR::staticRaiseError("Configuration type '$configType' is not registered in Config_Container::writeDatasrc.", null, PEAR_ERROR_RETURN);
        }
        $includeFile = $GLOBALS['CONFIG_TYPES'][$configType][0];
        $className = $GLOBALS['CONFIG_TYPES'][$configType][1];
        include_once($includeFile);
        ...
        if ($fp) {
            ...
            return PEAR::staticRaiseError('Cannot open datasource for writing.', 1, PEAR_ERROR_RETURN);
        }

...
define('MDB2_SCHEMA_ERROR_UNSUPPORTED', -4); 
define('MDB2_SCHEMA_ERROR_INVALID', -5); 
...
if (!empty($index['primary']) || !empty($index['unique'])) { $result = $this->db->manager->dropConstraint($table_name, $index_name); } else { $result = $this->db->manager->dropIndex($table_name, $index_name); }
...
if (!empty($changes)) {
 ...
 if ($create_index) { if (!empty($index['primary']) || !empty($index['unique'])) { $result = $this->db->manager->createConstraint($table_name, $index_name, $index); } else { $result = $this->db->manager->createIndex($table_name, $index_name, $index); } if (PEAR::isError($result)) { return $result; }
}
}
return MDB2_OK;
}  
...
return !empty($errorMessages[$value]) ?
 ...
 {
 $err =& PEAR::staticRaiseError(null, $code, $mode, $options, $userinfo, 'MDB2_Schema_Error', true);
return $err;
}  

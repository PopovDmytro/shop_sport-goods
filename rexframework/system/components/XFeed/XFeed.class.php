...
require_once dirname(__FILE__).DIRECTORY_SEPARATOR.'XPlugin.class.php';
 ...
 * @author   Fatal
 * @access   public
 * @package  XFeed.class.php
 * @created  Thu Sep 06 11:20:21 EEST 2007
 */
class XFeed extends XPlugin
{
	var $feed;
	var $version;
	...
	/**
	 * Constructor of XFeed
	 *
	 * @access  public
	 */
	function XFeed($aTitle, $aDescription, $aLink, $aSyndicationURL, $aItems, $aSource, $aAuthor)
	{
		parent::XPlugin('xfeed');
		...
		$this->link 			= $aLink;
		$this->syndicationURL 	= $aSyndicationURL;
		$this->items 			= $aItems;
		$this->source 			= $aSource;
		$this->author 			= $aAuthor;
	}
	...
	function getFeed()
	{
		...
		$rss->link = $this->link;
		$rss->syndicationURL = $this->syndicationURL;
		...
		for ($i=0, $size=sizeof($this->items); $i<$size; $i++) {
		    ...
		    $item->link = $this->items[$i]['url'];
		    $item->description = $this->items[$i]['description'];

		    //optional
		    //item->descriptionTruncSize = 500;
		    //item->descriptionHtmlSyndicated = true;

		    $item->date = $this->items[$i]['date']; ;
		    $item->source = $this->items[$i]['source']; ;
		    $item->author = $this->items[$i]['author']; ;

		    $rss->addItem($item);
		}

		//echo $rss->saveFeed("RSS1.0", "news/feed.xml");
		$this->feed = $rss->createFeed($this->version);
	}
 ...
 * Version string.
 **/
define("FEEDCREATOR_VERSION", "www.rexengine.com");

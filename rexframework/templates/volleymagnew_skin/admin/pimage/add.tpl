    ...
    <form method="post" enctype="multipart/form-data" action="" class="addform" id="addPicture">
        ...
        <table cellspacing="5" cellpadding="0" border="0" width="100%">
            <tr>
                ...
                <td class="frame-t-td">
                    <select name="entity[product_id]" data-autosave="true">
            ...
            <tr>
                ...
                <td class="frame-t-td" id="img-src-td">
                    ...
                    {/if}
                    <br/>
                    <input type="file" id="input-file" name="image" value="" data-autosave="true">
                </td>
            </tr>
            {if $colorAttr}
                <tr>
                    ...
                    <td class="frame-t-td">
                        <select class="filter-attr-add" name="entity[attribute_id]" data-autosave="true">
                            <option value="0">Нет</option>
                            {foreach from=$colorAttr key=key item=attribute}
                                <option value="{$attribute.id}" {if $parent_attr eq $attribute.id or $attribute.id eq 1}selected{/if}>{$attribute.name}</option>
                            {/foreach}
                        </select>
                    </td>
                </tr>
            ...
            <tr>
                ...
                <td class="frame-t-td">
                    <input type="text" name="entity[name]" data-autosave="true" maxlength="128" value="{if $entity->name}{$entity->name|escape:'html'}{else}{$product_name}{/if}">
                </td>
            </tr>
            <tr>
                <td class="frame-t-td-l">
                ...
                <td class="frame-t-td">
                    <textarea name="entity[description]" data-autosave="true" style="height: 200px; width: 400px;">{if $entity->description}{$entity->description}{/if}</textarea>
                </td>
            </tr>
        </table>
    </form>
        ...
        function getFilterColor()
        {
            $.rex('attr2Prod', 'filterAttribute', {attr_id: $('.filter-attr-add').val(), product_id: {/literal}{$product_id}{literal}}, function(data){
                ...
                if (data != false) {
                    ...
                    block += '<td class="frame-t-td"><select class="filter" data-autosave="true" name="entity[attribute_id]">';

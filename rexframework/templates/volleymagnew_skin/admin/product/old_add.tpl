        ...
        <table cellspacing="5" cellpadding="0" border="0" width="100%">
                ...
                <td class="frame-t-td" align="left">
                    <select name="entity[category_id]" id="cat">
                        {foreach from=$pcatalogList key=key item=category}
                            <option value="{$category.id}" {if $entity->category_id eq $category.id}selected{/if}>{if $category.level > 0}{section name=level loop=$category.level}-{/section}{/if}{$category.name}</option>
                        {/foreach}                
                    </select>
                </td>
            </tr>
            ...
            <div class="popup-all-path">
                ...
                <div style="float: left;">
                    ...
                    {*<select class="multiselect" multiple="multiple" name="related[]" id="prod_check" style="height: 100px;">*}
...
{if $view}
    <table class="frame-t-s" cellspacing="5" cellpadding="0" border="0" align="center">
        <tr valign="">
            <td class="frame-t-td-s" colspan="2"><button id="button" type="button" class="item_show ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" name="entity[submit]" ><span class="ui-button-text">Ok</span></button></td>
        </tr>
    </table>
{else}
    <table class="frame-t-s" cellspacing="5" cellpadding="0" border="0" align="center">
...
{literal}
    ...
    function load_brands() {
        $.rex('brand', 'loadByCat', {category_id: cat_select.val()}, function(data) {
            if (false !== data) {
                var html = '<select id="brnd" name="entity[brand_id]">';
                for (var i in data) {
                    if (i) {

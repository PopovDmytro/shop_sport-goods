        ...
        {*<div class="wrapper">*}
            <div class="holder one-fourth">
                ...
                <ul class="brand-list filter_choose_all_list">
                    ...
                    {foreach from=$categoryBrandList key=key item=currBrand}
                        <li class="ntitle">
                            <input type="checkbox" id="brand_{$currBrand.id}" name="filter[brand][]" value="{$currBrand.id}" {if !$withoutBrand or $currBrand.selected}checked{/if}>
                            <a href="{url mod=pCatalog task=$pcatalog->alias brand_alias=$currBrand.alias}">{$currBrand.name}</a>
                        </li>
                    {/foreach}
                </ul>
            </div>

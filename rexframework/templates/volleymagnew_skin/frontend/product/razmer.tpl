<div class="obyv-block">
    <a href="javascript:void(0)" style="float:right;" class="obyv-close">Закрыть</a>
    {*<img src="/content/file/-/17%5B1%5D.jpg">*}
    {if $productBrand->alias eq 'mizuno'}
    <div class="mizuno">
        <img src="/content/images/image/mizuno_logo.gif">
        <h3 class="men_obyv">Размерная сетка на спортивную обувь Mizuno</h3> 
        {*<h3 id="men_obyv">Мужская обувь Mizuno</h3> *}
        <table border=1>
            <tr>
                <th>Размер UK</th>
                <td>C10</td>
                <td>C10.5</td>
                <td>C11</td>
                <td>C11.5</td>
                <td>C12</td>
                <td>C12.5</td>
                <td>C13</td>
                <td>C13.5</td>
                <td>1</td>
                <td>1.5</td>
                <td>2</td>
                <td>2.5</td>
                <td>3</td>
                <td>3.5</td>
                <td>4</td>
                <td>4.5</td>
                <td>5</td>
                <td>5.5</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>27</td>
                <td>28</td>
                <td>28.5</td>
                <td>29</td>
                <td>30</td>
                <td>30.5</td>
                <td>31</td>
                <td>32</td>
                <td>32.5</td>
                <td>33</td>
                <td>34</td>
                <td>34.5</td>
                <td>35</td>
                <td>36</td>
                <td>36.5</td>
                <td>37</td>
                <td>38</td>
                <td>38.5</td>
            </tr>
            <tr>
                <th>Размеры в см.</th>
                <td>16.0</td>
                <td>16.5</td>
                <td>17.0</td>
                <td>17.5</td>
                <td>18.0</td>
                <td>18.5</td>
                <td>19.0</td>
                <td>19.5</td>
                <td>20.0</td>
                <td>20.5</td>
                <td>21.0</td>
                <td>21.5</td>
                <td>22.0</td>
                <td>22.5</td>
                <td>23.0</td>
                <td>23.5</td>
                <td>24.0</td>
                <td>24.5</td>
            </tr>
        </table>
        {*<h3>Женская обувь Mizuno</h3>*}
        <table border="1">
            <tr>
                <th>Размер UK</th>
                <td>6</td>
                <td>6.5</td>
                <td>7</td>
                <td>7.5</td>
                <td>8</td>
                <td>8.5</td>
                <td>9</td>
                <td>9.5</td>
                <td>10</td>
                <td>10.5</td>
                <td>11</td>
                <td>11.5</td>
                <td>12</td>
                <td>12.5</td>
                <td>13</td>
                <td>13.5</td>
                <td>14</td>
                <td>14.5</td>
                <td>15</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>39</td>
                <td>40</td>
                <td>40.5</td>
                <td>41</td>
                <td>42</td>
                <td>42.5</td>
                <td>43</td>
                <td>44</td>
                <td>44.5</td>
                <td>45</td>
                <td>46</td>
                <td>46.5</td>
                <td>47</td>
                <td>&nbsp</td>
                <td>49</td>
                <td>&nbsp</td>
                <td>50</td>
                <td>&nbsp</td>
                <td>51</td>
            </tr>
            <tr>
                <th>Размеры в см.</th>
                <td>25.0</td>
                <td>25.5</td>
                <td>26.0</td>
                <td>26.5</td>
                <td>27.0</td>
                <td>27.5</td>
                <td>28.0</td>
                <td>28.5</td>
                <td>29.0</td>
                <td>29.5</td>
                <td>30.0</td>
                <td>30.5</td>
                <td>31.0</td>
                <td>31.5</td>
                <td>32.0</td>
                <td>32.5</td>
                <td>33.0</td>
                <td>33.5</td>
                <td>34.0</td>
            </tr>
        </table>
    </div>
    {elseif $productBrand->alias eq 'asics'}
    <div class="asics">
        <img src="/content/images/image/asics_logo.jpg">
        <h2 class="men_obyv">Размерная сетка на обувь Asics</h2>
        <h3>Мужская обувь Asics</h3>
        <table border="1">
            <tr id="main-asics">
                <th>Размеры US</th>
                <td>4</td>
                <td>4,5</td>
                <td>5</td>
                <td>5,5</td>
                <td>6</td>
                <td>6,5</td>
                <td>7</td>
                <td>7,5</td>
                <td>8</td>
                <td>8,5</td>
                <td>9</td>
                <td>9,5</td>
                <td>10</td>
                <td>10,5</td>
                <td>11</td>
                <td>11,5</td>
                <td>12</td>
                <td>12,5</td>
                <td>13</td>
                <td>14</td>
                <td>15</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>34,5</td>
                <td>35,5</td>
                <td>36,0</td>
                <td>36,5</td>
                <td>37,5</td>
                <td>38,0</td>
                <td>38,5</td>
                <td>39,0</td>
                <td>40,0</td>
                <td>40,5</td>
                <td>41,0</td>
                <td>42,0</td>
                <td>42,5</td>
                <td>43,0</td>
                <td>43,5</td>
                <td>44,5</td>
                <td>45,0</td>
                <td>45,5</td>
                <td>46,5</td>
                <td>47,5</td>
                <td>49,0</td>
            </tr>
            <tr>
                <th>Размеры в см</th>
                <td>22,5</td>
                <td>23,0</td>
                <td>23,5</td>
                <td>24,0</td>
                <td>24,5</td>
                <td>25,0</td>
                <td>25,2</td>
                <td>25,5</td>
                <td>26,0</td>
                <td>26,5</td>
                <td>27,0</td>
                <td>27,5</td>
                <td>28,0</td>
                <td>28,2</td>
                <td>28,5</td>
                <td>29,0</td>
                <td>29,5</td>
                <td>30,0</td>
                <td>30,5</td>
                <td>31,0</td>
                <td>32,0</td>
            </tr>
        </table>
        <h3>Женская обувь Asics</h3>
        <table border="1">
            <tr id="main-asics">
                <th>Размеры US</th>
                <td>4</td>
                <td>4,5</td>
                <td>5</td>
                <td>5,5</td>
                <td>6</td>
                <td>6,5</td>
                <td>7</td>
                <td>7,5</td>
                <td>8</td>
                <td>8,5</td>
                <td>9</td>
                <td>9,5</td>
                <td>10</td>
                <td>10,5</td>
                <td>11</td>
                <td>11,5</td>
                <td>12</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>34,0</td>
                <td>34,5</td>
                <td>35,0</td>
                <td>35,5</td>
                <td>36,0</td>
                <td>36,5</td>
                <td>37,0</td>
                <td>37,5</td>
                <td>38,0</td>
                <td>38,5</td>
                <td>39,0</td>
                <td>40,0</td>
                <td>40,5</td>
                <td>41,0</td>
                <td>42,0</td>
                <td>42,5</td>
                <td>43,0</td>
            </tr>
            <tr>
                <th>Размеры в см</th>
                <td>21,5</td>
                <td>22,0</td>
                <td>22,5</td>
                <td>22,7</td>
                <td>23,0</td>
                <td>23,5</td>
                <td>24,0</td>
                <td>24,5</td>
                <td>25,0</td>
                <td>25,5</td>
                <td>25,7</td>
                <td>26,0</td>
                <td>26,5</td>
                <td>27,0</td>
                <td>27,5</td>
                <td>28,0</td>
                <td>28,5</td>
            </tr>
        </table>
        <h3>Детская обувь Asics</h3>
        <table border="1">
            <tr id="main-asics">
                <th>Размеры US</th>
                <td>K10</td>
                <td>K11</td>
                <td>K12</td>
                <td>K13</td>
                <td>1</td>
                <td>1,5</td>
                <td>2</td>
                <td>2,5</td>
                <td>3</td>
                <td>3,5</td>
                <td>4</td>
                <td>4,5</td>
                <td>5</td>
                <td>5,5</td>
                <td>6</td>
                <td>6,5</td>
                <td>7</td>
                <td>7,5</td>
                <td>8</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>25,5</td>
                <td>27,0</td>
                <td>28,5</td>
                <td>30,0</td>
                <td>31,0</td>
                <td>31,5</td>
                <td>32,0</td>
                <td>33,0</td>
                <td>33,5</td>
                <td>34,0</td>
                <td>34,5</td>
                <td>35,5</td>
                <td>36,0</td>
                <td>36,5</td>
                <td>37,5</td>
                <td>38,0</td>
                <td>38,5</td>
                <td>39,0</td>
                <td>40,0</td>
            </tr>
            <tr>
                <th>Размеры в см</th>
                <td>17,0</td>
                <td>17,5</td>
                <td>18,5</td>
                <td>19,5</td>
                <td>20,0</td>
                <td>20,5</td>
                <td>21,0</td>
                <td>21,5</td>
                <td>22,0</td>
                <td>22,2</td>
                <td>22,5</td>
                <td>23,0</td>
                <td>23,5</td>
                <td>24,0</td>
                <td>24,5</td>
                <td>25,0</td>
                <td>25,2</td>
                <td>25,5</td>
                <td>26,0</td>
            </tr>
        </table>
        <h3>Обувь Asics Унисекс</h3>
        <table border="1">
            <tr id="main-asics">
                <th>Размеры US</th>
                <td>4</td>
                <td>4,5</td>
                <td>5</td>
                <td>5,5</td>
                <td>6</td>
                <td>6,5</td>
                <td>7</td>
                <td>7,5</td>
                <td>8</td>
                <td>8,5</td>
                <td>9</td>
                <td>9,5</td>
                <td>10</td>
                <td>10,5</td>
                <td>11</td>
                <td>11,5</td>
                <td>12</td>
                <td>12,5</td>
                <td>13</td>
                <td>14</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>34,0</td>
                <td>34,5</td>
                <td>35,0</td>
                <td>36,0</td>
                <td>36,5</td>
                <td>37,0</td>
                <td>38,0</td>
                <td>38,5</td>
                <td>39,5</td>
                <td>40,5</td>
                <td>41,0</td>
                <td>41,5</td>
                <td>42,5</td>
                <td>43,0</td>
                <td>44,0</td>
                <td>44,5</td>
                <td>45,0</td>
                <td>46,0</td>
                <td>47,0</td>
                <td>48,0</td>
            </tr>
            <tr>
                <th>Размеры в см</th>
                <td>22,0</td>
                <td>22,5</td>
                <td>23,0</td>
                <td>23,5</td>
                <td>24,0</td>
                <td>24,5</td>
                <td>25,0</td>
                <td>25,5</td>
                <td>26,0</td>
                <td>26,5</td>
                <td>27,0</td>
                <td>27,5</td>
                <td>28,0</td>
                <td>28,5</td>
                <td>29,0</td>
                <td>29,5</td>
                <td>30,0</td>
                <td>30,5</td>
                <td>31,0</td>
                <td>32,0</td>
            </tr>
        </table> 
    </div>
    {/if} 
</div>
<div class="odezhda-block">
    <a href="javascript:void(0)" style="float:right;" class="odezhda-close">Закрыть</a>
    {*<img src="/content/images/image/2.jpg">*}
    {if $productBrand->alias eq 'mizuno'}
    <div class="mizuno mizuno_odezhda">
        <img src="/content/images/image/mizuno_logo.gif">
        <h3>Размерная сетка на одежду Mizuno</h3>
        <table border="1">
            <tr>
                <th>Men's</th>
                <th>S</th>
                <th>M</th>
                <th>L</th>
                <th>XL</th>
                <th>XXL</th>
            </tr>
            <tr>
                <td>обхват груди(см)</td>
                <td>89-96</td>
                <td>97-104</td>
                <td>105-112</td>
                <td>113-120</td>
                <td>121-127</td>
            </tr>
            <tr>
                <td>обхват талии(см)</td>
                <td>73-80</td>
                <td>81-88</td>
                <td>89-96</td>
                <td>97-104</td>
                <td>105-111</td>
            </tr>
            <tr>
                <td>обхват бёдер(см)</td>
                <td>89-96</td>
                <td>97-104</td>
                <td>105-112</td>
                <td>113-120</td>
                <td>121-127</td>
            </tr>
            <tr>
                <td>рост(см)</td>
                <td>170-175</td>
                <td>175-180</td>
                <td>180-185</td>
                <td>185-190</td>
                <td>190-195</td>
            </tr>
        </table>
        <table border="1">
            <tr>
                <th>Women's</th>
                <th>S</th>
                <th>M</th>
                <th>L</th>
                <th>XL</th>
            </tr>
            <tr>
                <td>обхват груди(см)</td>
                <td>82-88</td>
                <td>89-95</td>
                <td>96-102</td>
                <td>103-109</td>
            </tr>
            <tr>
                <td>обхват талии(см)</td>
                <td>61-67</td>
                <td>67-73</td>
                <td>73-79</td>
                <td>79-85</td>
            </tr>
            <tr>
                <td>обхват бёдер(см)</td>
                <td>85-91</td>
                <td>92-98</td>
                <td>99-105</td>
                <td>106-112</td>
            </tr>
            <tr>
                <td>рост(см)</td>
                <td>163-168</td>
                <td>168-173</td>
                <td>173-178</td>
                <td>178-183</td>
            </tr>
        </table>
        <table border="1">
            <tr>
                <th>Junior</th>
                <th>S</th>
                <th>M</th>
                <th>L</th>
                <th>XL</th>
                <th>XXL</th>
            </tr>
            <tr>
                <td>рост(см)</td>
                <td>104</td>
                <td>116</td>
                <td>128</td>
                <td>140</td>
                <td>152</td>
            </tr>
            <tr>
                <td>возраст</td>
                <td>4</td>
                <td>6</td>
                <td>8</td>
                <td>10</td>
                <td>12</td>
            </tr>
        </table> 
    </div>
    {elseif $productBrand->alias eq 'asics'}
    <div class="asics">
        <img src="/content/images/image/asics_logo.jpg">
        <h2 class="men_obyv">Размерная сетка на одежду Asics</h2>
        <h3>Спортивная одежда для мужчин</h3>
        <table border="1">
            <tr id="main-asics">
                <th>Размер US</th>
                <td>S</td>
                <td>M</td>
                <td>L</td>
                <td>XL</td>
                <td>XXL</td>
                <td>XXXL</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>44-46</td>
                <td>46-48</td>
                <td>48-50</td>
                <td>50-52</td>
                <td>54-56</td>
                <td>56-58</td>
            </tr>
            <tr>
                <th>Рост(см)</th>
                <td>174-178</td>
                <td>178-182</td>
                <td>182-186</td>
                <td>186-190</td>
                <td>190-194</td>
                <td>194-198</td>
            </tr>
            <tr>
                <th>Объем груди(см)</th>
                <td>83-89</td>
                <td>89-95</td>
                <td>95-101</td>
                <td>101-107</td>
                <td>107-113</td>
                <td>113-119</td>
            </tr>
            <tr>
                <th>Объем талии(см)</th>
                <td>73-79</td>
                <td>79-85</td>
                <td>85-91</td>
                <td>91-97</td>
                <td>97-103</td>
                <td>103-109</td>
            </tr>
            <tr>
                <th>Объем бедер(см)</th>
                <td>90,5-95,5</td>
                <td>95,5-100,5</td>
                <td>100,5-105,5</td>
                <td>105,5-110,5</td>
                <td>110,5-115,5</td>
                <td>115,5-120,5</td>
            </tr>
        </table>
        <h3>Спортивная одежда для женщин</h3>
        <table border="1">
            <tr id="main-asics">
                <th>Размер US</th>
                <td>XS</td>
                <td>S</td>
                <td>M</td>
                <td>L</td>
                <td>XL</td>
                <td>XXL</td>
            </tr>
            <tr>
                <th>Размер по ДСТУ</th>
                <td>40-42</td>
                <td>42-44</td>
                <td>44-46</td>
                <td>46-48</td>
                <td>48-50</td>
                <td>50-52</td>
            </tr>
            <tr>
                <th>Рост(см)</th>
                <td>&nbsp</td>
                <td>160-164</td>
                <td>164-168</td>
                <td>168-172</td>
                <td>172-176</td>
                <td>176-180</td>
            </tr>
            <tr>
                <th>Объем груди(см)</th>
                <td> </td>
                <td>71-77</td>
                <td>77-83</td>
                <td>83-89</td>
                <td>89-95</td>
                <td>95-101</td>
            </tr>
            <tr>
                <th>Объем талии(см)</th>
                <td>&nbsp</td>
                <td>51,5-58,5</td>
                <td>58,5-65,5</td>
                <td>65,5-72,5</td>
                <td>72,5-79,5</td>
                <td>79,5-86,5</td>
            </tr>
            <tr>
                <th>Объем бедер(см)</th>
                <td>&nbsp</td>
                <td>80-86</td>
                <td>86-92</td>
                <td>92-98</td>
                <td>98-104</td>
                <td>104-110</td>
            </tr>
        </table>
    </div>
    {elseif $productBrand->alias eq 'speedo'}
        <div>
            <p>
                <img alt="" src="/content/images/brand/21/icon.png" style="width: 95px; height: 60px;margin: 0 auto; display: block;position: relative;">
            </p>
            <h3 style="text-align: center;">Таблица соответствия размеров</h3>
            <p>
                <img alt="" style="position:relative;" src="/content/file/-/speedo-woman-size.jpg">
                <img alt="" style="position:relative;" src="/content/file/-/speedo-man-size.jpg">
            </p>
        </div>
    {/if}  
</div>
<div class="nakolenniki-block">
    <a href="javascript:void(0)" style="float:right;" class="nakolenniki-close">Закрыть</a>
    {*<img src="/content/images/image/1.png">*}
    <div class="mizuno mizuno_nakolenniki">
        {*<img src="/content/images/image/mizuno_logo.gif">*}
        <h3>Таблица размеров наколенников</h3>
        <p>Размер наколенника определяется по центру коленной чашечки</p>
        <table border="1">
            <tr>
                <th id="{if $productBrand->alias eq 'asics'}main-asics{elseif $productBrand->alias eq 'mizuno'}main-mizuno{/if}">Размер "S"</th>
                <td>30-35 см</td>
            </tr>
            <tr>
                <th id="{if $productBrand->alias eq 'asics'}main-asics{elseif $productBrand->alias eq 'mizuno'}main-mizuno{/if}">Размер "M"</th>
                <td>35-40 см</td>
            </tr>
            <tr>
                <th id="{if $productBrand->alias eq 'asics'}main-asics{elseif $productBrand->alias eq 'mizuno'}main-mizuno{/if}">Размер "L"</th>
                <td>40-45 см</td>
            </tr>
            <tr>
                <th id="{if $productBrand->alias eq 'asics'}main-asics{elseif $productBrand->alias eq 'mizuno'}main-mizuno{/if}">Размер "XL"</th>
                <td>45-50 см</td>
            </tr>
        </table>
    </div> 
</div>
<div class="noski-block">
    <a href="javascript:void(0)" style="float:right;" class="noski-close">Закрыть</a>
    {if $productBrand->alias eq 'mizuno'}
    <div class="mizuno mizuno-noski">
        <img src="/content/images/image/mizuno_logo.gif">
        <h3>Размерная сетка на носки Mizuno</h3>
        <table border="1">
            <tr>
                <th>Размер носков</th>
                <th>Размер обуви</th>
            </tr>
            <tr>
                <td>S</td>
                <td>35-37</td>
            </tr>
            <tr>
                <td>M</td>
                <td>38-40</td>
            </tr>
            <tr>
                <td>L</td>
                <td>41-43</td>
            </tr>
            <tr>
                <td>XL</td>
                <td>44-46</td>
            </tr>
        </table>
    </div>
    {elseif $productBrand->alias eq 'asics'}
    
    <div class="asics">
        <img src="/content/images/image/asics_logo.jpg">
        <h3>Размерная сетка на носки Asics</h3>
        <table border="1">
            <tr>
                <th>Размер носков</th>
                <th>Размер обуви</th>
            </tr>
            <tr>
                <td>XS</td>
                <td>31-34</td>
            </tr>
            <tr>
                <td>S</td>
                <td>35-38</td>
            </tr>
            <tr>
                <td>M</td>
                <td>39-42</td>
            </tr>
            <tr>
                <td>L</td>
                <td>43-46</td>
            </tr>
            <tr>
                <td>XL</td>
                <td>47-49</td>
            </tr>
        </table>
    </div>
    {/if}
</div>

<div class="block-background"></div>
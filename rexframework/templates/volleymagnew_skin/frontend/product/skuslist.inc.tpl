{if $attrForSale}
    ...
    {foreach from=$attrForSale key=key item=attrFS}
        {if $key neq 'Цвет'}
            <dl class="product-card_dl attr-wrapper size">
                <dt>
                    <span>{$key}:</span>
                </dt>
                <dd>
    ...
    <li class="colors-slider-wrapper">
    {foreach from=$attrForSale key=key item=attrFS}
        {if $key eq 'Цвет'}
            <div class="colors-slider_box">
                <div class="label">
                    <span class="choice-color">
                        {if $navCategoryList.0.alias eq 'sportivnoe-pitanie'}Выберите вкус:{else}Выберите цвет:{/if}
                    </span>
                </div>
                <div class="product-slider">
                    <div class="my-slider my-slider_product-colors attr-wrapper slider4">
                        {foreach from=$attrFS key=k item=attr}
                            <div class="slide-attr my-sliders_slide product-colors_slide" title="{$attr.name}">
                                <a href="javascript:void(0);" class="img-holder text-center attr-default color-box" data-sku-article="{$attr.sku_article}" attr_id="{$attr.attribute_id}" id="{$attr.id}">
                                    {if $attr.img_id}<img class="slide-img" src="{getimg type=mini name=pImage id=$attr.img_id ext=$attr.img_ext}" title="{$attr.name}">{else}{$attr.name}{/if}
                                </a>
                            </div>
                        {/foreach}
                    </div>
